...
with mlflow.start_run(run_name="hyperparameter_tuning",description="Best hyperparameter trained RF Model") as parent:
    ...
    for i in range(len(grid_search.cv_results_['params'])):
        with mlflow.start_run(nested=True) as child:
            ...
            signature=mlflow.models.infer_signature(X_train,grid_search['param'][i].predict(X_train))
            mlflow.sklearn.log_model(grid_search['param'][i],"random_forest",signature=signature)
    ...
    train_df=mlflow.data.from_pandas(train_df)
    test_df=mlflow.data.from_pandas(test_df)
    mlflow.log_input("training",train_df)
    mlflow.log_input("testing",test_df)

    signature=mlflow.model.infer_signature(X_train,grid_search.best_estimator_.predict(X_train))
